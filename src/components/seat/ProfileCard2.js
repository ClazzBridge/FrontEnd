...
export default function StudentRoom() {
  ...
  const fetchSeatsByCourse = async (courseId) => {
    try {
      ...
      socket.on("fetchedStudentData", (data) => {
        redis_response = data;
        console.log(redis_response, "redis"); // 확인용 로그
      ...
      console.log(sortedSeats, "sortedSeats");
  ...
  useEffect(() => {
    // 이해도 상태 업데이트를 위한 웹소켓 리스너
    socket.on("understanding", (data) => {
      console.log(data, "data=========");
      setProfiles((prevProfiles) => {
        return prevProfiles.map((profile) => {
          console.log(profile, "profile");
          if (profile.member && profile.member.id === currentUser?.memberId) {
            return {
              ...profile,
              member: {
                understanding: data.isUnderstanding,
              },
            };
          }
          return profile;
        ...
        >
          {profiles.map((profile, index) => (
            <ProfileCard
              ...
              isUnderstanding={profile.member?.isUnderstanding ?? false}
              isHandRaised={profile.member?.isHandRaised ?? false}
              isSelf={
